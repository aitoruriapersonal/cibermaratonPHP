...
foreach ($casos as $caso) {
    $inicioIteracion = microtime(true);
    ...
    if (!$participante || empty($participante->nick)) {
        ...
        $log->info(Constantes::MENSAJE_ESPERANDO . round($tiempoRestante) . Constantes::MENSAJE_SEGUNDOS_SIGUIENTE_ITERACION);
        $finIteracion = microtime(true);
        $tiempoRestante = 120 - $tiempoTranscurrido;
        if ($tiempoRestante > 0) {
            $log->info(Constantes::MENSAJE_ESPERANDO . round($tiempoRestante) . Constantes::MENSAJE_SEGUNDOS_SIGUIENTE_ITERACION);
            sleep((int)ceil($tiempoRestante));
        }
        continue;
    }
    ...
    if ($stats) {
        $statsApiModel = ChesscomPlayerStatsApiConverter::toChesscomPlayerStats($stats, $participante, $username);
    ...
    if ($numApi === $numResultados) {
        ...
        // Espera para la siguiente iteración
        $finIteracion = microtime(true);
        $tiempoTranscurrido = $finIteracion - $inicioIteracion;
        $tiempoRestante = 120 - $tiempoTranscurrido;
        if ($tiempoRestante > 0) {
            $log->info("Esperando " . round($tiempoRestante) . " segundos antes de la siguiente iteración.");
            sleep((int)ceil($tiempoRestante));
        }
        continue;
    }
